...
final class Spotify: ObservableObject {
    @Published var userTags: OrderedSet<String> = []
    
    @Published var songTags: [String : OrderedSet<String>] = [:]

    @Published var savedTracks: [Track] = []
    @Published var savedAlbums: [Album] = []
    @Published var savedPlaylists: [Playlist<PlaylistItemsReference>] = []
    
    @Published var trackCache: OrderedSet<Track> = []
    ...
    let loginCallbackURL = URL(
        string: "messing-with-spotify://login-callback"
    )!
    ...
    let keychain = Keychain(service: "com.notpr.messing-with-spotify")
    ...
    @Published var loadingSavedTracks: Bool = false
    @Published var failedLoadingSavedTracks: Bool = false

    @MainActor
    func loadSavedTracks() async throws {
        do {
            loadingSavedTracks = true
            failedLoadingSavedTracks = false
            ...
            try await api
                ...
                .extendPages(api)
                .async() { AllTracks in
                    let Tracks = AllTracks.items
                        .map(\.item)
                        .filter { $0.id != nil }

                    self.savedTracks.append(contentsOf: Tracks)
                    self.trackCache.append(contentsOf: Tracks)

                }
            loadingSavedTracks = false
        } catch {
            failedLoadingSavedTracks = true
            loadingSavedTracks = false
        }
    }

    @Published var loadingSavedAlbums: Bool = false
    @Published var failedLoadingSavedAlbums: Bool = false
    
    @MainActor
    func loadSavedAlbums() async throws {
        do {
            loadingSavedAlbums = true
            failedLoadingSavedAlbums = false
            
            savedAlbums = []
            
            try await api
                .currentUserSavedAlbums()
                .extendPages(api).async() { Container in
                    let Albums = Container.items
                        .map(\.item)
                        .filter { $0.id != nil }
            ...
            failedLoadingSavedAlbums = true
            loadingSavedAlbums = false
        }
    }
    
    @Published var loadingSavedPlaylists: Bool = false
    @Published var failedLoadingSavedPlaylists: Bool = false
    
    @MainActor
    func loadSavedPlaylists() async throws {
        do {
            loadingSavedPlaylists = true
            failedLoadingSavedPlaylists = false
            ...
            try await api
                ...
                .extendPages(api)
                .async() { PlaylistContainer in
                    self.savedPlaylists.append(contentsOf: PlaylistContainer.items)
                }
            loadingSavedPlaylists = false
        } catch {
            failedLoadingSavedPlaylists = true
            loadingSavedPlaylists = false
        }
    }
    
    @MainActor
    func loadPlaylistItems(playlist: Playlist<PlaylistItemsReference>, onDataReceived: @escaping ([Track]) -> Void) async throws {
        try await api
            .playlistTracks(playlist)
            .extendPages(api)
            .async() { PlaylistTracks in
                let Tracks = PlaylistTracks.items
                    .compactMap(\.item)
                
                onDataReceived(Tracks)
            }

    }
    
    @MainActor
    func clearPlaylist(playlist: String) async throws {
        _ = try await api.replaceAllPlaylistItems(playlist, with: []).async()
    }
    
    @MainActor
    func addTracksToPlaylist(playlist: String, uris: [String]) async throws {
        if uris.count <= 100 {
            let _ = try await api.addToPlaylist(playlist, uris: uris).async()
            return
        }
        
        let chunks = uris.chunked(size: 100)
        
        for chunk in chunks {
            let _ = try await api.addToPlaylist(playlist, uris: chunk).async()
        }
    }    
}
